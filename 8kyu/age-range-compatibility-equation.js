...
function datingRange(age) {
    ...
    if (age > 14) {
        minAge = String(Math.floor(age / 2 + 7));
        maxAge = String(Math.floor((age - 7) * 2));
        recomendedAge = minAge + '-' + maxAge;
    } else {
        minAge = String(Math.floor(age - 0.10 * age));
        maxAge = String(Math.floor(age + 0.10 * age));
        recomendedAge = minAge + '-' + maxAge;
    }
    return recomendedAge;
}

console.log('datingRange: ', datingRange(27))